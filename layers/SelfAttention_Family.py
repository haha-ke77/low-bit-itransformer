...
from utils.quant_utils import QuantLinear
...
import numpy as np
from math import sqrt
# 确保引入你之前定义的 STEQuantizer
from utils.quant_utils import STEQuantizer


class FullAttention(nn.Module):
    def __init__(self, mask_flag=True, factor=5, scale=None, attention_dropout=0.1,
                 output_attention=False, bits=8, quantize_attn_map=True):
        ...
        self.dropout = nn.Dropout(attention_dropout)
        self.bits = bits
        self.quantize_attn_map = quantize_attn_map
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # only quantize attn map for student
        if self.quantize_attn_map and self.bits < 32:
            A = STEQuantizer.apply(A, self.bits)
...
# 修改 AttentionLayer 的 __init__ 方法
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, bits=8, use_quant=True):
        ...
        if hasattr(self.inner_attention, 'bits'):
            self.inner_attention.bits = bits
        if hasattr(self.inner_attention, 'quantize_attn_map'):
            self.inner_attention.quantize_attn_map = use_quant

        if use_quant:
            self.query_projection = QuantLinear(d_model, d_keys * n_heads, bits=bits, weight_bits=bits, enable_quant=True)
            self.key_projection   = QuantLinear(d_model, d_keys * n_heads, bits=bits, weight_bits=bits, enable_quant=True)
            self.value_projection = QuantLinear(d_model, d_values * n_heads, bits=bits, weight_bits=bits, enable_quant=True)
            self.out_projection   = QuantLinear(d_values * n_heads, d_model, bits=bits, weight_bits=bits, enable_quant=True)
        else:
